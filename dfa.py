...
class DFA:
    ...
    def load(self, source: str, is_file: bool = True) -> dict[str, object]:
        ...
        if data['initial_state'] not in self.states:
            errors.append(f"Initial state '{data['initial_state']}' not found.")
        else:
        ...
        for accepting in data['accepting_states']:
            if accepting not in self.states:
                errors.append(f"Accepting state '{accepting}' not found.")
            else:
        ...
        for state, transitions in data['delta'].items():
            if state not in self.states:
                errors.append(f"State '{state}' in transition function not found.")
                continue
            for symbol, dest in transitions.items():
                if dest not in self.states:
                    errors.append(f"State '{dest}' in transition function not found.")
                    continue
                self.states[state].transition[symbol] = self.states[dest]

        # verify DFA
        if self.alphabet is None:
            errors.append("Alphabet not defined.")
        else:
